...
def selectMultiCombo(moles, prev_attacked, rho, past_classes, curr_classes, all_classes, safe, no_double):
    ...
    for i in past_classes: #attacked
        for j in curr_classes: #confounding
            ...
            spread_inv = moles[all_classes.index(i)][all_classes.index(j)][:, all_classes.index(j)]
            pc = np.percentile(spread, p)
            pc_inv = np.percentile(spread_inv, p)
            if safe:
                if pc_inv - pc <= 0.1:
