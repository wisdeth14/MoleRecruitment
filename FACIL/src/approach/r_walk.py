...
class Appr(Inc_Learning_Appr):
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        if len(self.exemplars_dataset) > 0 and t > 0:
            ...
            #to handle the moles
            if len(trn_loader.dataset) < len(self.exemplars_dataset):
                exemplars_subset = deepcopy(self.exemplars_dataset)

                sub_no = int(len(trn_loader.dataset) / 2.25) #preserves normal ratio
                spacing = int(len(self.exemplars_dataset) / sub_no)
                i = 0
                indices = []
                while len(indices) < sub_no:
                    indices.append(i)
                    i += spacing

                exemplars_subset.images = [exemplars_subset.images[i] for i in indices]
                exemplars_subset.labels = [exemplars_subset.labels[i] for i in indices]

                trn_loader = torch.utils.data.DataLoader(trn_loader.dataset + exemplars_subset,
