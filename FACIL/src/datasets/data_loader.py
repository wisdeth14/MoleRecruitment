...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, validation=.1):
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        # datasets
        trn_dset, val_dset, tst_dset, total_set, curtaskcla, classes, mapping, mapping_id = get_datasets(cur_dataset, dc['path'], num_tasks, nc_first_task,
        ...
        for tt in range(num_tasks):
            ...
            tst_load.append(data.DataLoader(tst_dset[tt], batch_size=batch_size, shuffle=False, num_workers=num_workers,
                                            pin_memory=pin_memory))
    return trn_load, val_load, tst_load, total_set, taskcla, classes, mapping, mapping_id, trn_transform
...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    trn_dset, val_dset, tst_dset, total_set = [], [], [], None
    classes, mapping, mapping_id = None, None, None
    ...
    if 'mnist' in dataset:
        ...
        total_set = deepcopy(tvcifar_trn)
        total_set.data = np.concatenate((tvcifar_trn.data, tvcifar_tst.data))
        total_set.targets = tvcifar_trn.targets + tvcifar_tst.targets

        trn_data = {'x': tvcifar_trn.data, 'y': tvcifar_trn.targets}
        tst_data = {'x': tvcifar_tst.data, 'y': tvcifar_tst.targets}
        classes = tvcifar_trn.classes
        # compute splits
        all_data, taskcla, class_indices, mapping, mapping_id = memd.get_data(trn_data, tst_data, classes, validation=validation,
                                                         num_tasks=num_tasks, nc_first_task=nc_first_task,
                                                         shuffle_classes=class_order is None, class_order=class_order)
    ...
    #     path = '/scratch/ewisdom'
    #     tvilsvrc_trn = TorchVisionImageNet(path, split='train', download=True)
    #     tvilsvrc_tst = TorchVisionImageNet(path, split='val', download=True)
    #     print("here")
    #     quit()
        ...
        for c in range(len(class_order)):
            mapping[c] = classes[class_order[c]]
            mapping_id[class_order[c]] = c
            #mapping[c] = classes[c]
    ...
    return trn_dset, val_dset, tst_dset, total_set, taskcla, classes, mapping, mapping_id
